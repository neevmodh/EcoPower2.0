...
function drawBrandHeader(doc, subtitle, rightLine1, rightLine2) {
  ...
  doc.text(subtitle || 'Energy as a Service Platform', 14, 23);
  if (rightLine1) {
...
export async function downloadDashboardReport({ user, telemetry, invoices, carbon }) {
  ...
  const kpis = [
    ...
    ['Trees Equivalent', String(carbon?.total_trees_equivalent || 0)],
