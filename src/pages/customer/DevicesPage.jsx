...
const DevicesPage = () => {
    ...
    const toggleDevice = async (id, currentStatus) => {
        ...
        try {
            setCsvData(prev => ({
                ...prev,
                devices: prev.devices.map(d => d.id === id ? { ...d, status: newStatus } : d)
            }));
    ...
    const applySchedule = async (id, scheduleOn, scheduleOff) => {
        try {
            setCsvData(prev => ({
                ...prev,
                devices: prev.devices.map(d => d.id === id ? { ...d, scheduleOn, scheduleOff } : d)
            }));
        ...
        try {
            setCsvData(prev => {
                ...
                for (const item of optimizationStats.recommendations) {
                    newDevices = newDevices.map(d => d.id === item.deviceId ? { ...d, scheduleOn: item.rec.recommendedOn, scheduleOff: item.rec.recommendedOff } : d);
                }
